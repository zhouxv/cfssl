...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	if len(c.peerCertificates) > 0 {
		...
		if certVerify.hasSignatureAndHash {
			...
			case *pqc.PublicKey:
				signatureAndHash.signature = signaturePQC
			}
		}
		...
		case *pqc.PublicKey:
			if signatureAndHash.signature != signaturePQC {
				err = errors.New("bad signature type for client's PQC certificate")
				break
			}
			//zhouxvcheck
		}
...
func (hs *serverHandshakeState) processCertsFromClient(certificates [][]byte) (crypto.PublicKey, error) {
	...
	if len(certs) > 0 {
		...
		case *ecdsa.PublicKey, *rsa.PublicKey, *pqc.PublicKey:
			pub = key
		default:
