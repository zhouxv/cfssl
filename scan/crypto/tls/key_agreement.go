...
NextCandidate:
	...
	case signaturePQC:
		_, ok := priv.Public().(*pqc.PublicKey)
		if !ok {
			return nil, errors.New("ECDHE PQC requires a PQC server key")
		}
	default:
...
func (ka *ecdheKeyAgreement) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, skx *serverKeyExchangeMsg) error {
	...
	case signaturePQC:
		_, ok := cert.PublicKey.(*pqc.PublicKey)
		if !ok {
			return errors.New("ECDHE PQC requires a PQC server public key")
		}
		//zhouxvcheck
		// if err := rsa.VerifyPKCS1v15(pubKey, hashFunc, digest, sig); err != nil {
		// 	return err
